...
{	
	...
	public class HashModem
	{
		...
		public static const PERMITTED_SYMBOLS:String = " 0123456789АаБбВвГгДдЕеЖжЗзИиЙйКкЛлМмНнОоПпРрСсТтУуФфХхЦцЧчШшЩщЬьЫыЪъЭэЮюЯяAaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz!@():,./-_?" + "\n" + '"';
		...
		public static function modulate(data:String):String{
			trace("HashModem.modulate data=" + data);
			...
			trace("HashModem.modulate hash=" + hash);
			return hash;
		}
		
		/**
		 * Проверяет данные на вхождение неразрешенных символов.
		 * Возвращает true если данные пригодны для кодирования.
		...
		/**
		 * По символу(ам) hash возвращает число. Может вводиться 1 или 2 символа
		 * 1 символ - число 0..35
		 * 2 символа - число 36..1000 [первый символ - старший, т.е. определяет большую значащую часть числа]
		 * (АВТОМАТИЧЕСКИ ОТНИМАЕТ 36 ОТ "ДВУХСИМВОЛЬНОГО" ЧИСЛА - ТЕМ САМЫМ ВОЗВРАЩАЯ ID ENGINE)
		 */
		public static function charsToKey(chars:String):int{
			if (chars.length == 1){
				return fragmentCharToKey(chars);
			}else if(chars.length == 2){
				return 35 * fragmentCharToKey(chars.charAt(0))  +  fragmentCharToKey(chars.charAt(1)) - 36;
			}
			throw new Error("Unexpected chars quantity = " + chars.length);
		}
		
		/**
		 * По числу возвращает символ(ы) hash - 1 или 2 символа
		 * long - генерировать 1 или 2 символа
		 * long=true	0..35 - 1 символ
		 * long=false	0..1000 - 2 символа
		 */
		public static function keyToChars(key:int, long:Boolean = false):String{
			if (!long && key < 36)
				return fragmentKeyToChar(key);
			else{
				key += 36;
				return fragmentKeyToChar(int(key/35)) + fragmentKeyToChar(key % 35);
			}
		}
		
		/**
		 * Замененяет все "неразрешенные" символы
		 */
		public static function checkSymbols(data:String):String{
			var response:String = "";
			for(var i:int = 0;i<data.length;i++){
				var symbol:String = data.charAt(i);
				if(HashModem.PERMITTED_SYMBOLS.indexOf(symbol) == -1){
					if(symbol == "ё") symbol = "е";
					else if(symbol == "Ё") symbol = "Е";
